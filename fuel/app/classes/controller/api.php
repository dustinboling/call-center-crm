    ...
    class Controller_Api extends Controller{
        ...
        function action_receive_payment($k = null){
            ...
            if(isset($payment['transaction_id'])){
                $payment['transaction_id'] = preg_replace('/[^0-9]/','',$payment['transaction_id']);
            }
            ...
            if(!empty($payment['schedule'])){
                $payment['sort'] = preg_replace('/[^0-9]/','',$payment['schedule']);
            }
            ...
            if(count($errors)){
                ...
                if(!empty($payment['rep_id'])){
                    $rep = Model_System_User::findByField('salesforce_id', $payment['rep_id']);
                    if(!empty($rep['id'])){
                        $payment['sales_rep_id'] = $rep['id'];
                    }
                }
                
                if(isset($payment['rep_id'])){
                    unset($payment['rep_id']);
                }
                
                if(empty($payment['sales_rep_id'])){
                ...
                $exists = Model_SFPayment::findByTransactionID($payment['transaction_id']);
                if($exists){
                    Model_SFPayment::updateByTransactionID($payment['transaction_id'], $payment);
                }else{
                    Model_SFPayment::add($payment);
                }
