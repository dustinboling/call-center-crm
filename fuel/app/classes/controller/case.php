<?php
    ...
    class Controller_Case extends Controller_Base{
        ...
        function action_listing(){
            ...
            if(isset($_SESSION['filter']['limit']) && !empty($_SESSION['filter']['limit'])){
                $this->limit = $_SESSION['filter']['limit'];
                if(isset($_GET['page'])){
        ...
        function action_batch_process(){
            ...
            if(!empty($_POST)){
                ...
                if(isset($_POST['cases']) && !empty($_POST['cases'])){
                    ...
                    if($_POST['distribution_group_id']){
                        
                        try{
                            $message = Model_Case::distributeToGroup($_POST['cases'], $_POST['distribution_group_id']);
                            notify::success($message);
                        }catch(Exception $e){
                            notify::error($e);
                        }
        ...
        function action_update($id){
            ...
            if(Model_Account::getType() == 'User'){
                if(!in_array($data['case']['sales_rep_id'], array($_SESSION['user']['id'],0))){
                    Notify::error("You don't have access to this case");
                    Response::redirect('/case/listing');
                }
            }
            ...
            $data['options'] = Model_System_ObjectFields::findAllOptions(1);
            $data['campaigns'] = Model_System_Campaign::findAll();
            $data['events'] = Model_Event::findByCase($id);
            $data['users'] = Model_System_User::findByDepartment(array('sales', 'agent'));
        ...
        function action_run_action($id){
            ...
            try{
                ...
                Model_Action::run($id, $_POST['action_id'], $_POST['note'], $_SESSION['user']['id']);
                notify::success('Case Updated');
            }catch(Exception $e){
                notify::error($e);
            }
        ...
        function action_save_quick_update($id){
            if(!empty($_POST)){
                ...
                if($run_action){
                    Model_Action::run($id, $action_id, $action_note, $_SESSION['user']['id']);
                }
            }
