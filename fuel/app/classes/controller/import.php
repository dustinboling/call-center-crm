<?php
    ...
    class Controller_Import extends Controller_Base{
        ...
        protected $import_path = '/var/www/crm/import/';
        ...
        function action_appointments($csv_filename, $run=0){
            ...
            if($file !== false){
                while (($data = fgetcsv($file)) !== FALSE) {
                    ...
                    if (empty($data[1]) || empty($data[0]) || empty($data[1]) || strtotime($data[2]) < time()) {
                        $row++;
                        continue;
                    }
                    ...
                    $event = array(
                                        'case_id' => $data[0],
                                        'user_id' => Model_Import::findRepByName($data[1]),
                                        'at' => date('Y-m-d H:i:s', strtotime($data[2])),
                                        'alert_offset' => 0,
                                        'alert_email' => 1,
                                        'alert_popup' => 1,
                                        );
                    ...
                    if ($run) {
                        ...
                        echo 'Event for '.$data[0].' processed<br/>';
                    } else {
                        echo '<p>';
                        echo '<pre>';
                        print_r($event);
