    ...
    class Model_Case extends Model{
        ...
        static function findAll($offset = 0, $limit = 100, $sort_field = 'created', $order = 'desc'){
            ...
            $result = DB::select('c.*', array('s.name', 'status'), array(DB::expr('CONCAT(u.first_name, " ", u.last_name)'), 'sales_rep_name'))
                    ->from(array('cases','c'))
                    ->join(array('statuses','s'))->on('s.id', '=', 'c.status_id')
        ...
        static function quickSearch($query){
            ...
            $mdb = Mongo_Db::instance();
            $mdb->or_where(array('_id' => (int)$query, 'primary_phone' => $phone_query, 'secondary_phone' => $phone_query, 'mobile_phone' => $phone_query));
        ...
        static function find($id){
            ...
            $result = DB::select('c.*', array('s.name', 'status'))->from(array('cases','c'))->join(array('statuses','s'))->on('s.id', '=', 'c.status_id')->where('c.id', '=', $id)->execute();
        ...
        static function findByIDs($ids){
            ...
            $result = DB::select('c.*', array('s.name', 'status'))->from(array('cases','c'))->join(array('statuses','s'))->on('s.id', '=', 'c.status_id')->where('c.id', 'in', $ids)->execute();
            return self::buildResult($result);
