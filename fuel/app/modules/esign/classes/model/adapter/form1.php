<?php
    ...
    class Model_Adapter_Form1 extends Model_Adapter_Form{
        ...
        static function processData($case_id, $data){
            ...
            if(isset($case['federal_not_filed']) && is_array($case['federal_not_filed'])){ $tax_years = array_merge($tax_years, $case['federal_not_filed']); }
            if(isset($case['federal_owed']) && is_array($case['federal_owed'])){ $tax_years = array_merge($tax_years, $case['federal_owed']); }
            if(isset($case['state_not_filed']) && is_array($case['state_not_filed'])){ $tax_years = array_merge($tax_years, $case['state_not_filed']); }
            if(isset($case['state_owed']) && is_array($case['state_owed'])){ $tax_years = array_merge($tax_years, $case['state_owed']); }
            
            $unique_tax_years = array_unique($tax_years);
            
            $form_data['years_owed'] = implode(',', $unique_tax_years);
